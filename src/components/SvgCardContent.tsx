...
const SvgCardContent: React.FC<Props> = ({ onLinkClick, isMobile = false }) => {
  ...
  const LEFT = isMobile ? 120 : 140;                // left padding
  const RIGHT_EDGE = isMobile ? 1500 : 1560;        // right visual edge
  const SAFE_RIGHT = isMobile ? 1480 : 1540;        // values sit slightly inside
  const NAME_Y = isMobile ? 330 : 340;
  const NAME_SIZE = isMobile ? 155 : 165;           // smaller, consistent spacing
  ...
  // Name width was previously used for fit-to-width. Removing to avoid distorted spacing.
  ...
  return (
    ...
    >
      <rect width="100%" height="100%" fill="#fff" data-darkreader-ignore />
      <g fontFamily="'Satoshi', Arial, sans-serif" fill="#000" data-darkreader-ignore>
        {/* Role */}
        ...
        {/* Natural spacing (no fit-to-width) to avoid squished/over-spaced glyphs */}
        <text x={LEFT} y={NAME_Y} fontSize={NAME_SIZE} fontWeight={700} textRendering="geometricPrecision" className="svg-name">
          TONY T
